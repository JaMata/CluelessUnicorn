...
var CLIENT_LIST = {};
var PLAYER_LIST = [];
var gameStatus = 'Create a Match';
var players = {"Candy Floss Angel Ears": {"id": 0, "position": [0, 3]}, 
			"Cookie Rainbow Dancer": {"id": 1, "position": [1, 0]}, 
			"Sunny Cloverfield": {"id": 2, "position": [1, 4]}, 
			"Cupcake Sugarsocks": {"id": 3, "position": [3, 0]}, 
			"Jingles Fizzle Flanks": {"id": 4, "position": [4, 1]}, 
			"Princess Cloudburst": {"id": 5, "position": [4, 3]}};
var taken = {};
...
io.on('connection', function(client) {
	...
	client.on('join', function() {
		gameStatus = "Matchmade";             // Change the status of the game
	...
	client.on('character', function(data) {
		if (gameStatus !== 'Match In-Progress') {
	...
	client.on('newPosition', function(data) {
		data.push(client.character);
		emitToPlayers('move', data);
	});
	...
	client.on('suggest', function(data) {
		if(data[0].board) {
			emitToPlayers('suggestion', {"suggestion": data, "name": client.character});
			if (data[0].suspect != client.character) {
				emitToPlayers('movedActivity', {"suggestion": data});
			}
	...
	client.on('moved', function(data) {
		emitToPlayers('suggestion', data);
		emitToPlayers('movedActivity', data);
		var id = setChallenger();
		CLIENT_LIST[id].emit('prove');
		challengerCount++;
	});
	...
	client.on('sendProof', function(data) {
		...
		var others = getOthers(PLAYER_LIST[currentTurn]);
		for(var i = 0; i < others.length; i++) {
			CLIENT_LIST[others[i]].emit('proofActivity', client.character);
		}
	});
	...
	client.on('accuse', function(data) {
		accusationCount++;
		emitToPlayers('accusationMade', [data, client.character]);
		if (accusationCount < PLAYER_LIST.length) {
			client.emit('accusationResponse', [checkAccusation(data), secretEnvelope]);
			emitToOthers(client.id, 'falseAccusation', client.character);
		} else {
    ...
    client.on('disconnect', function(data) {
    	...
    	if (gameStatus === 'Matchmade' && client.character) {
	...
	client.on('sendMessage', function(data) {
        emitToPlayers('receiveMessage', data);
	});
...
function resetGame() {
    ...
    players = {"Candy Floss Angel Ears": {"id": 0, "position": [0, 3]}, 
              "Cookie Rainbow Dancer": {"id": 1, "position": [1, 0]}, 
              "Sunny Cloverfield": {"id": 2, "position": [1, 4]}, 
              "Cupcake Sugarsocks": {"id": 3, "position": [3, 0]}, 
              "Jingles Fizzle Flanks": {"id": 4, "position": [4, 1]}, 
...
function leaveGame(id, character, hand) {
    ...
    if (PLAYER_LIST.length >= 3 && gameStatus === 'Match In-Progress') {
    ...
    } else if (PLAYER_LIST.length < 3 && gameStatus === 'Match In-Progress') {
        emitToPlayers('insufficient');
        resetGame();
    }
}

function emitToOthers(id, msg, data=null) {
	var others = getOthers(id);
	for(var i = 0; i < others.length; i++) {
		if (data !== null) {
			CLIENT_LIST[others[i]].emit(msg, data);
		}
		else {
			CLIENT_LIST[others[i]].emit(msg);
		}
	}
}